...
// VARIABLES DEL MODO SELECCIÓN Y SUSTAIN
let modoD = false;
...
function cambiarZoomDesdeAjustes(valor) {
    ...
    if (container) {
        container.style.transform = `scale(${globalZoomFactor})`;
    }
    
    const pLine = document.getElementById('playback-line');
    if(pLine) {
        pLine.style.width = `calc(45px * 8 * ${globalZoomFactor} + 16px)`;
    }
}
...
function generarEstructuraGrilla(filas) {
    ...
    for (let c = 0; c < 4; c++) {
        ...
        for (let f = 0; f < filas; f++) {
            ...
            cell.id = `cell-${f}-${c}`;
            cell.onclick = () => alternarNota(f, c, cell);
...
// FUNCIONES DE CONTROL DE LA HERRAMIENTA SELECCIÓN
function activarModoD() { modoD = true; document.getElementById('btn-tool-d').classList.add('active'); }
function desactivarModoD() { modoD = false; document.getElementById('btn-tool-d').classList.remove('active'); }

function alternarNota(f, c, cell) {
    const id = `cell-${f}-${c}`;
    const key = `${f}-${c}`;

    if (modoD) {
        // MODO SELECCIÓN: Marcamos la nota para estirarla si es que existe en el Dom
        if (cell.querySelector('.grid-note-circle')) {
            if (notaSeleccionada) {
    ...
    // MODO NORMAL: Crear o Borrar nota de tu JSON metadata
    if (currentChartData.notes[key]) {
        delete currentChartData.notes[key];
        cell.innerHTML = '';
        if (notaSeleccionada === id) notaSeleccionada = null;
    } else {
        // Guardamos como objeto interno para soportar el atributo 'len'
...
function ajustarLongitudNota(dir) {
    if (!notaSeleccionada) return alert("¡Primero selecciona una nota usando el botón D!");
    
    const cell = document.getElementById(notaSeleccionada);
    if (!cell) return;
    
    const parts = notaSeleccionada.replace('cell-', '').split('-');
    const f = parts[0]; const c = parts[1];
    const key = `${f}-${c}`;
    
    if (!currentChartData.notes[key]) return;
    
    // Si la estructura vieja guardó solo 'true', la convertimos a objeto
    if (typeof currentChartData.notes[key] !== 'object') {
        currentChartData.notes[key] = { len: 0 };
    }
    
    let len = currentChartData.notes[key].len || 0;
    len = Math.max(0, len + dir); // ▲ suma longitud, ▼ resta longitud
...
function cargarDatosEnMesa(v1, v2, name, bpm) {
    document.getElementById('track-player-label').innerText = v1 ? (v1.name || "Cargado") : "No Player Voice";
    document.getElementById('track-enemy-label').innerText = v2 ? (v2.name || "Cargado") : "No Enemy Voice";
...
function togglePlayPause() {
    ...
    if(isPlaying) {
        ...
        const tiempoExacto = audioInst.currentTime;
        if(audioVoice1) audioVoice1.currentTime = tiempoExacto;
        if(audioVoice2) audioVoice2.currentTime = tiempoExacto;

        audioInst.play();
        if(audioVoice1) audioVoice1.play();
        if(audioVoice2) audioVoice2.play();
...
function manejarScrollManual() {
    ...
    const stepDuration = (60 / currentChartData.bpm) / 4;
    const nuevoStep = posicionPxReal / alturaCelda;
    let nuevoTiempo = nuevoStep * stepDuration;
...
function menuArchivarChart() { 
    ...
    alert(`¡Chart "${currentChartData.songName}" archivado con éxito!`);
    renderizarProyectosArchivados();
}
...
function eliminarProyectoArchivado(event, id) {
    ...
    if(!confirm("¿Seguro que quieres eliminar este chart de tu memoria interna?")) return;
...
function cargarProyectoDesdeArchivo(id) {
    ...
    currentChartData = proy;
    limpiarAudiosExistentes();

    alert("Proyecto cargado. Recuerda configurar los audios desde el menú de carga si es necesario.");
    ...
    for(let key in currentChartData.notes) {
        ...
        if(cell) {
            ...
            // Redibujar la línea de sustain si tiene longitud guardada
...
function procesarArchivoFNFC(input) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (data && data.songName) {
                ...
                for(let key in currentChartData.notes) {
                    ...
                    if(cell) {
                        ...
                        // Redibujar sustain desde archivo .fnfc importado
                        const noteData = currentChartData.notes[key];
                        if (noteData && noteData.len > 0) {
                            const line = document.createElement('div');
                            line.className = 'sustain-line';
            ...
            alert("Error al leer el archivo .fnfc");
        }
    };
